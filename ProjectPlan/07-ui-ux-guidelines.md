\# Leonessa App

\## UI / UX Guidelines v1.0



\---



\# 1. Obiettivo



Definire l'identità visiva e le linee guida di design della Leonessa App.



L'app deve trasmettere:



\- Professionalità

\- Appartenenza

\- Competizione

\- Modernità

\- Esclusività



L'obiettivo è creare un'esperienza che faccia sentire ogni studente parte della Leonessa Cup.



\---



\# 2. Brand Identity



\## Personalità del Brand



Leonessa Cup deve apparire:



\- Giovane

\- Ambiziosa

\- Sportiva

\- Tecnologica

\- Premium

\- Inclusiva



\---



\## Sensazioni da Trasmettere



Quando un utente apre l'app deve percepire:



```text

Questa non è una semplice app scolastica.



Questa è LA piattaforma ufficiale della Leonessa Cup.

```



\---

# 2.1 Identità Territoriale e Sportiva

## Obiettivo

Leonessa Platform non deve apparire come una dashboard SaaS o un semplice software gestionale.

L'app deve comunicare immediatamente:

- Brescia
- Leonessa Cup
- Competizione
- Appartenenza
- Comunità studentesca
- Evento sportivo

L'utente deve percepire di entrare nell'ecosistema ufficiale della Leonessa Cup.

---

## Principio Fondamentale

Quando un utente apre l'app deve pensare:

"Sto entrando nella Leonessa Cup."

e non:

"Sto utilizzando un software."

---

## Personalità Visiva

La piattaforma deve essere percepita come:

- Competitiva
- Giovane
- Premium
- Territoriale
- Coinvolgente
- Sportiva
- Moderna
- Ambiziosa

---

## La V Bresciana

### Concetto

L'identità calcistica bresciana è fortemente associata alla storica "V" bianca.

Leonessa Platform può ispirarsi a questo linguaggio grafico senza utilizzarlo come logo.

### Regole

NON utilizzare la V come lettera decorativa.

NON sostituire il logo ufficiale.

NON creare loghi alternativi.

UTILIZZARE la V come principio geometrico.

### Applicazioni Consentite

- tagli diagonali nelle card
- separatori di sezione
- pattern di sfondo
- shape decorative
- hero background
- animazioni di ingresso
- loading screen
- indicatori di progressione

### Obiettivo

Creare un linguaggio grafico immediatamente riconoscibile e legato al territorio bresciano.

L'utente non deve necessariamente notare la V, ma percepirla inconsciamente attraverso il design.

---

## Atmosfera Sportiva

### Obiettivo

Trasmettere l'energia di un evento sportivo reale.

### Elementi Consentiti

- glow blu ispirato all'illuminazione degli stadi
- profondità visiva
- contrasti netti
- pattern ispirati alle gradinate
- elementi grafici dinamici

### Elementi da Evitare

- stile corporate
- dashboard aziendale
- estetica fintech
- cyberpunk
- effetti gaming aggressivi
- design generici da startup

---

## Utilizzo della Leonessa

### Principio

Il logo non deve essere l'unico elemento identitario.

### Elementi Derivati Consentiti

- silhouette della leonessa
- criniera stilizzata
- pattern derivati dalle curve del simbolo
- elementi decorativi a bassissima opacità

### Regola

Questi elementi devono essere percepiti e non necessariamente notati.

Devono rafforzare il brand senza distrarre l'utente.

---

## Hero Sections

Le schermate principali devono trasmettere:

- orgoglio
- competizione
- appartenenza
- identità cittadina

### Esempi di Copy

22 scuole. Una sola città.

La competizione delle scuole di Brescia.

Ogni partita conta.

La tua scuola. La tua città. La tua Cup.

---

## Test di Riconoscibilità

Nascondendo il logo Leonessa Cup, la schermata deve continuare a trasmettere:

- competizione
- appartenenza
- Brescia
- evento sportivo

Se la schermata perde completamente la propria identità senza il logo, il design deve essere rivisto.

\# 3. Logo



\## Logo Ufficiale



Logo Leonessa Cup:



\- Sempre ben visibile.

\- Utilizzato preferibilmente in bianco.

\- Mai deformato.

\- Mai ruotato.

\- Mai utilizzato con colori differenti senza approvazione.



\---



\## Safe Area



Lasciare sempre spazio libero attorno al logo.



Minimo:



```text

1x altezza testa leonessa

```



\---



\# 4. Palette Colori



\## Colori Primari



\### Leonessa Blue



```css

\#0D1B60

```



Utilizzo:



\- CTA principali

\- Header

\- Bottom Navigation

\- Elementi premium



\---



\### White



```css

\#FFFFFF

```



Utilizzo:



\- Testi

\- Card

\- Contrasto



\---



\## Colori Secondari



\### Navy Dark



```css

\#071034

```



Utilizzo:



\- Background principale

\- Modal

\- Overlay



\---



\### Electric Blue



```css

\#305CFF

```



Utilizzo:



\- Hover

\- Focus

\- Elementi attivi



\---



\## Colori Neutri



\### Gray 100



```css

\#F5F7FA

```



\### Gray 200



```css

\#E8EDF5

```



\### Gray 400



```css

\#A0A7B5

```



\### Gray 700



```css

\#3C4454

```



\---



\# 5. Modalità Tema



\## Tema Principale



Dark Mode.



La Leonessa App deve essere progettata principalmente in dark mode.



\---



\## Tema Secondario



Light Mode.



Supportato ma non prioritario.



\---



\# 6. Tipografia



\## Typography System



\### Font roles



Utilizzo:



\- Bebas Neue: Hero, heading, statistiche evidenziate, numeri e KPI importanti.

\- Inter: testo normale, descrizioni, form, navigazione e UI generale.

\- La tipografia display non deve essere usata per paragrafi o controlli operativi.

\- La tipografia body deve restare leggibile, con peso e contrasto sufficienti.

### Design tokens

```css
--font-family-display: "Bebas Neue", Impact, sans-serif;
--font-family-body: "Inter", "Helvetica Neue", Arial, sans-serif;
--type-display-size: clamp(3rem, 12vw, 6rem);
--type-h1-size: clamp(2.5rem, 8vw, 4rem);
--type-h2-size: clamp(1.75rem, 5vw, 2.5rem);
--type-h3-size: clamp(1.35rem, 4vw, 1.75rem);
--type-body-size: 1rem;
--type-caption-size: 0.875rem;
```

I token display usano peso `400`, perché Bebas Neue è una famiglia
condensata pensata per mantenere impatto anche senza pesi aggiuntivi.


\---



\## Font Fallback



```css

"Inter", "Helvetica Neue", Arial, sans-serif
```



\---



\# 7. Gerarchia Tipografica



\## Display (Bebas Neue)



```css

var(--type-display-size)

400

```



Utilizzo:



\- Hero

\- Eventi speciali



\---



\## H1 (Bebas Neue)



```css

var(--type-h1-size)

400

```



\---



\## H2 (Bebas Neue)



```css

var(--type-h2-size)

400

```



\---



\## H3 (Bebas Neue)



```css

var(--type-h3-size)

400

```



\---



\## Body (Inter)



```css

var(--type-body-size)

400

```



\---



\## Caption (Inter)



```css

var(--type-caption-size)

400

```



\---



\# 8. Principi UX



\## Mobile First



Tutte le schermate devono essere progettate prima per smartphone.



\---



\## Massimo 3 Tocchi



Le azioni più frequenti devono essere raggiungibili in massimo:



```text

3 tap

```



\---



\## Velocità



L'utente non deve attendere inutilmente.



Target:



```text

< 1 secondo

```



per ogni navigazione principale.



\---



\## Chiarezza



Ogni schermata deve avere:



\- un obiettivo;

\- una CTA principale;

\- una gerarchia visiva evidente.



\---



\# 9. Layout System



\## Grid



```css

8px spacing system

```



\---



\## Border Radius



\### Small



```css

8px

```



\### Medium



```css

16px

```



\### Large



```css

24px

```



\---



\## Padding Standard



```css

16px

```



\---



\# 10. Componenti Base



\## Primary Button



Background:



```css

\#0D1B60

```



Testo:



```css

\#FFFFFF

```



Utilizzo:



\- azione principale.



\---



\## Secondary Button



Border:



```css

1px solid #0D1B60

```



Background:



```css

transparent

```



\---



\## Ghost Button



Solo testo.



Utilizzo:



\- azioni secondarie.



\---



\# 11. Cards



Le card rappresentano il componente principale dell'app.



\---



\## Match Card



Contiene:



\- squadre

\- risultato

\- data

\- stato



\---



\## Mission Card



Contiene:



\- titolo

\- reward

\- progresso



\---



\## School Card



Contiene:



\- logo

\- nome scuola

\- posizione classifica



\---



\## Reward Card



Contiene:



\- premio

\- costo

\- disponibilità



\---



\# 12. Iconografia



\## Stile



\- Outline

\- Rounded

\- Modern



\---



\## Libreria



Consigliata:



```bash

lucide-react

```



\---



\# 13. Animazioni



\## Filosofia



Animazioni eleganti e leggere.



Mai eccessive.



\---



\## Tecnologia



```text

Framer Motion

```



\---



\## Durate



\### Hover



```css

150ms

```



\---



\### Transition



```css

250ms

```



\---



\### Page Transition



```css

300ms

```



\---



\# 14. Microinterazioni



\## Like



Piccolo bounce.



\---



\## Badge



Animazione reward.



\---



\## Missione Completata



Confetti minimali.



\---



\## Guadagno Punti



Animazione contatore.



\---



\# 15. Navigazione



\## Bottom Navigation



Sempre presente.



```text

🏠 Home

🏆 Cup

🎯 Community

⭐ Rewards

👤 Profile

```



\---



\## Staff



Se utente STAFF:



```text

🛠 Staff

```



\---



\## Admin



Se ADMIN:



```text

⚙ Admin

```



\---



\# 16. Home Screen



Ordine consigliato:



```text

Hero

↓

Partita in evidenza

↓

News

↓

Missioni

↓

Ranking

↓

Eventi

```



\---



\# 17. Match Center



Obiettivo:



Far sembrare ogni partita importante.



\---



\## Componenti



\- score live

\- eventi

\- MVP

\- statistiche



\---



\# 18. Community



Obiettivo:



Creare coinvolgimento.



\---



\## Elementi



\- Ranking

\- Badge

\- Missioni

\- Activity Feed



\---



\# 19. Accessibilità



\## Contrasto



Minimo:



```text

WCAG AA

```



\---



\## Touch Target



Minimo:



```css

44x44px

```



\---



\## Leggibilità



Mai testo inferiore a:



```css

14px

```



\---



\# 20. Tone of Voice



L'app deve parlare come una community.



\---



\## Caratteristiche



\- Motivante

\- Positiva

\- Coinvolgente

\- Diretta

\- Mai infantile



\---



\## Esempio



❌ Hai completato la missione.



✅ Grande! Hai guadagnato 50 LP per la tua scuola.



\---



# 21. Design Inspirations

## Product Inspirations

- OneFootball
- Sofascore
- FotMob
- Formula 1 App
- Duolingo (gamification)
- Discord (community)

## Visual Inspirations

- Nike Football
- UEFA Champions League
- Serie A
- Brescia Calcio
- Identità sportive territoriali
- Eventi sportivi studenteschi

## Esperienza Obiettivo

L'app deve essere percepita come:

"La Champions League delle scuole di Brescia"

e non come:

"Un gestionale scolastico".


\# 22. KPI UX



Monitorare:



\- Tempo medio nell'app

\- Retention

\- Sessioni giornaliere

\- Completamento missioni

\- Click CTA

\- Conversione staff



\---

# 22.1 Regola Anti-SaaS

Ogni schermata deve evitare l'aspetto di:

- CRM
- Gestionale
- Dashboard aziendale
- Startup fintech

L'identità Leonessa deve essere riconoscibile anche senza logo.

Le forme, la composizione, l'atmosfera e il linguaggio visivo devono comunicare:

- competizione
- appartenenza
- Brescia
- sport

\# 23. Regola Fondamentale



Ogni schermata deve rispettare questa domanda:



```text

Questa schermata fa sentire l'utente parte della Leonessa Cup?

```



Se la risposta è no, va riprogettata.



\---
